...
export default async function handler(req, res) {
  ...
  if (!cardId || signature === undefined || pageIndex === undefined) return res.status(400).json({ error: "cardId, pageIndex and signature required" });
  ...
  if (pageIndex < 0 || pageIndex >= pages.length) return res.status(400).json({ error: "pageIndex out of range" });
