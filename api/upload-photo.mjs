...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!process.env.NEXT_PUBLIC_SUPABASE_URL) return res.status(500).json({ error: "Missing NEXT_PUBLIC_SUPABASE_URL" });
    if (!process.env.SUPABASE_SERVICE_KEY) return res.status(500).json({ error: "Missing SUPABASE_SERVICE_KEY" });

    const match = photo.match(/^data:(image\/[\w+]+);base64,(.+)$/s);
    if (!match) return res.status(400).json({ error: "Invalid photo — must be data URL" });
